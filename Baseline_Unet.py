...
def train(args, dataset, model, optimizer, loss, val_metric):
    ...
    for epoch in range(args.epoch):
        ...
        for i_batch, sample_batched in enumerate(dataloader_train):
            ...
            loss_seg_ = loss(input=output, target=mask) # focal will use sigmoid in loss function...
...
if __name__ == '__main__':
    ...
    loss2 =  monai.losses.FocalLoss(alpha=0.75, gamma=2.0).to(device) 
    # loss3 = monai.losses.Dice(sigmoid=False).to(device) 
    # loss4 = cl.ContextualLoss(use_vgg=True, vgg_layer='relu5_4',band_width=0.8).to(device)  # loss_con = Context_crit(pred_3C, mask_3C)
    # loss5: Cl+FL
    
    val_metric = monai.metrics.DiceHelper(sigmoid=True)  # sigmoid + 0.5 threshold

    train(args, dataset, model, optimizer, loss2, val_metric)
